...
spacing, the file stops being theirs. So this writes the way FineSub's
``secrets.py`` writes ``.env``: read, replace the one line that owns the
setting, write the rest back untouched.

Vendored from FineSub 0.4.2's ``finesub_bootstrap/config_file.py``, which the
0.5.0 desktop split deleted along with the desktop that was its only caller.
Nonoka X is now that caller, so the module lives here; ``write_atomic`` and
``holding_lock`` are still upstream's and are imported rather than copied.

Deliberately limited to scalar values. Rewriting an array means re-serializing
a container, which is where a line-based editor would start losing the user's
line breaks and inline comments -- so it refuses instead of corrupting, and the
day a UI needs it, that is the moment to bring in a round-trip TOML library
rather than to extend this.
"""
...
def _parsed_value(parsed: Mapping[str, Any], table: str, key: str) -> Any:
    """Read back ``table.key`` from a parsed document, dotted names included.

    The header `[llm.preferred_targets]` is one line to the writer above and
    one *name* to `_table_spans`, but `tomllib` nests it -- so a flat lookup
    reads every dotted table as absent and the verification below would undo a
    write that in fact took effect.
    """

    node: Any = parsed
    for segment in (*table.split("."), key):
...
def update_config_file(
    ...
    with holding_lock(_lock_path(path), timeout=_LOCK_TIMEOUT_SECONDS):
        ...
        for table, settings in updates.items():
            for key, value in settings.items():
                actual = _parsed_value(parsed, table, key)
